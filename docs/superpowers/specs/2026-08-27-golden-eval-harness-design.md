...
- Chunks with `chunk_size=300, overlap=50` — smaller than the app's production default (`chunk_size=1000`). Verified by hand: at the default size, this fixture (five pages, ~300-400 characters each) collapses into just two chunks spanning most of the document, making "did retrieval find the expected page" almost trivially true regardless of retrieval quality. At `chunk_size=300`, the fixture chunks into six pieces that map closely to one-per-page, which is what makes the retrieval check meaningful. This is a harness-only parameter — it does not change `chunk_pages_to_chunks`'s or `load_and_chunk`'s defaults.
- Embeds, and builds an **in-memory** `FaissStore` — this eval index is not registered through Phase 1's `paper_registry` (`register_paper`/`index_path_for`), since it is a throwaway evaluation artifact, not a real user paper, and registering it would pollute the "Previously indexed papers" list a real user sees in the sidebar.
- For each question, runs `make_orchestrator(store, embedder, top_k=2).invoke(build_initial_state(question))` and captures the final state. `top_k=2` (the app's default is 5) because this fixture only has six chunks total — at the default, retrieval would return nearly the whole document for every question, making the retrieval check trivially true regardless of relevance. Verified by hand: with `chunk_size=300`/`top_k=2`, four representative questions across different pages all correctly retrieved a chunk covering their expected page.
